...
const supportedLocales = PUBLIC_I18N_SUPPORTED_LOCALES.split(',').map(s => s.trim()).filter(s => s.length > 0);
...
export const setLocaleCookie = (event: RequestEvent, locale: string): void => {
  if (supportedLocales.includes(locale) && locale !== PUBLIC_I18N_DEFAULT_LOCALE) {
